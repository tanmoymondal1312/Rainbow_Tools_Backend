(function () {
    ...
    const ENDPOINT = '/image-optimization/remove-bg/';
    const STAGES = ['Uploading', 'Removing background', 'Finishing up'];
    const STAGE_DELAYS_MS = [0, 600, 2200];
    ...
    function addResultCard(name, dataUrl, ok, message) {
        ...
        if (ok) {
            card.innerHTML =
                '<div class="result-preview checkerboard"><img src="' + dataUrl +
                '" alt="' + escapeHtml(name) + ' with background removed"></div>' +
                ...
                escapeHtml(nameWithoutExt(name)) + '-no-bg.png">Download</a>' +
                '</div></div>';
        } else {
            card.innerHTML =
    ...
    async function processOne(file, index, total) {
        ...
        try {
            ...
            addResultCard(file.name, 'data:image/png;base64,' + data.images[0], true);
        } catch (err) {
